...
def run():
    ...
    # Start at the end
    start = [(start_x, start_y) for start_y, row in enumerate(terrain) for start_x, v in enumerate(row) if v == "E"][0]

    return HeightMap().find_path(terrain, start)


class HeightMap():

    def find_path(self, terrain, start):
        ...
        self.min_path = self.width * self.height
    ...
    def update_distance(self, landmark, x, y):
        if 0 <= x < self.width and 0 <= y < self.height:
            neighbour = self.terrain[y][x]
            if landmark.height - neighbour.height <= 1 and neighbour in self.open_list:
                dist = landmark.distance + 1
                if dist < neighbour.distance:
                    neighbour.distance = dist
                    # Remember the shortest path from end to 'a'
                    if neighbour.height == 0:
                        self.min_path = min(self.min_path, neighbour.distance)
